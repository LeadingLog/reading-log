...
public interface ReadingRecordRepository extends JpaRepository<ReadingRecord, Long> {
	...
	Integer findTotalReadingTimeByUserIdAndYear(@Param("userId") Integer userId, @Param("year") Integer year);
	 ...
	 //사용자 아이디 & 독서 일자 중 연 > 월 > 책 > 읽은시간 
	  @Query(
			    value = "SELECT " +
			            "EXTRACT(YEAR FROM r.read_date) AS year, " +
			            "EXTRACT(MONTH FROM r.read_date) AS month, " +
			            "r.book_id, " +
			            "SUM(r.total_time) AS total_time " +
			            "FROM reading_record r " +
			            "WHERE r.user_id = :userId " +
			            "GROUP BY year, month, r.book_id " +
			            "ORDER BY year, month",
			    nativeQuery = true
			)
			...
			@Query(
				    value = "SELECT " +
				            ...
				            "FROM reading_record r " +
				            "WHERE r.user_id = :userId " +
				            "GROUP BY r.book_id, year, month " +
				            "ORDER BY r.book_id, year, month",
				    nativeQuery = true
				)
			...
			@Query(
				    value = "SELECT r.book_id, SUM(r.total_time) AS total_time " +
				            "FROM reading_record r " +
				            "WHERE r.user_id = :userId " +
				            "AND EXTRACT(YEAR FROM r.read_date) = :year " +
				            "AND EXTRACT(MONTH FROM r.read_date) = :month " +
				            "GROUP BY r.book_id " +
				            "ORDER BY total_time DESC",
				    nativeQuery = true
				)
				List<Object[]> findExactYearMonthBookTotalTimeByUserId(
